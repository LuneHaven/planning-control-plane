...
long-running planning context as work moves across branches, decisions,
implementation stages, and AI sessions. It turns a planning process that used
to live — and rot — inside long chat transcripts into a persistent
**Planning Graph** in your repository, and projects it as a deterministic,
offline static dashboard.
...
This is not primarily a task-management problem. A regular task tracker
does not solve it, because the question is not "who is doing what" — it is
"where did the context and the boundaries of the discussion go". PCP
manages the structure, progress and context of the planning process
itself, not task assignment.

## The Core Idea

1. **Planning data is the source; HTML is a projection.** The Planning Graph
...
2. **Decisions and boundaries cascade down the tree.** Nodes form a
   planning tree (`PROGRAM → PHASE → STRATEGY → …`). Frozen decisions and
   scope boundaries made at a parent are *inherited and displayed* in every
   child node, so they stay visible instead of being re-litigated.
3. **The current focus is always resumable.** One node is the current focus.
...
4. **Output is deterministic and fully offline.** Same planning source +
   same PCP version = byte-identical output. The generated site references
   no CDN and no remote fonts, and makes no network requests at all; it
...
- **Frozen / Open / Blocking / Deferred Decisions** — categorized,
  cascaded down the tree, never silently lost
...
- **Bilingual UI** — English and 简体中文, switchable at runtime in the
  browser
- **Authority Boundary** — PCP owns planning only; it
...
- **Idea layer** — `.planning/ideas/` captures uncommitted thinking with
  ...
  triages it, and a malformed idea file can never block the plan
...
Planning nodes are the committed part of the control plane: a node exists
because the work has earned a place in the plan. The idea layer holds what
comes before that — captured thinking that has not yet been committed.
...
- **A single entry point.** An idea enters the planning graph only by graduating:
...
- The **sidebar** shows the full planning tree, including status, focus
...
session needs and nothing unrelated to the current work. The node page's
**Resume This Work** panel shows the same capsule with a copy button.
...
**Current release: V0.1.3** (published on
[PyPI](https://pypi.org/project/planning-control-plane/)). It is a usable
MVP validated through real-project self-use: the engine, CLI, validator,
capsule and bilingual UI all work and are covered by an automated test
suite (409 tests).
